...
`r fontawesome::fa_i("lightbulb")` **Cet exercice s'inspire du 
...
nous allons utiliser le package `r fontawesome::fa_i("r-project")` 
...
spécifiques à un compendium/package `r fontawesome::fa_i("r-project")`. 

Le package `r fontawesome::fa_i("r-project")` `rcompendium` s'inspire en grande partie du package `r fontawesome::fa_i("r-project")` [`rrtools`](https://github.com/benmarwick/rrtools)
...
à chaque ouverture de `r fontawesome::fa_i("r-project")` et son contenu sera
accessible aux fonctions du package `rcompendium`.

Redémarrez la session `r fontawesome::fa_i("r-project")` et vérifiez que vos
...
[instructions](https://rdatatoolbox.github.io/instructions.html) 
...
Le fichier `DESCRIPTION` décrit les métadonnées du projet (titre, auteur, description, dépendances requises, etc.). C'est un des éléments essentiels d'un package `r fontawesome::fa_i("r-project")`. Ici, nous allons le _détourner_ pour l'utiliser dans le cadre d'un compendium afin de bénéficier des outils de développement de packages `r fontawesome::fa_i("r-project")`.
